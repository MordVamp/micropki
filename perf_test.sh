...
./micropki ca init --subject "CN=Perf Root CA" --key-type rsa --key-size 4096 \
...
for i in {1..1000}; do
    ./micropki client gen-csr --subject "CN=user$i" --out-key ./perf_out/user$i.key.pem --out-csr ./perf_out/user$i.csr.pem > /dev/null 2>&1
    ./micropki ca issue-cert --ca-cert ./perf_pki/certs/ca.cert.pem --ca-key ./perf_pki/private/ca.key.pem \
